...
class condGANTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                ######################################################
                data = data_iter.next()
                imgs, fore_imgs, seg_imgs, captions, cap_lens, class_ids, keys = prepare_data(data)
                ...
                for i in range(len(netsD)):
                    netsD[i].zero_grad()
                    errD, log = discriminator_loss(netsD[i], imgs[i], fore_imgs[i], seg_imgs[i], fake_imgs[i],
    ...
    def sampling(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            R = np.zeros(30000)
            cont = True
            for ii in range(30):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                if (cont == False):
                    break
